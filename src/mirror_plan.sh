...
read_plan() {
    local input=$(cat)
    local file_path=$(printf '%s' "$input" | jq -r '.tool_input.file_path // empty')
...
get_mirror_dest() {
    ...
    local dest_filename=$(sed -n "s/.*${PLAN_MIRROR_PATTERN}.*/\1/p" "$file" | head -1)
...
get_plan_path() {
    local file_path="$1"
    local plans_dir="$(pwd)/$LOCAL_PLANS_FOLDER"
    mkdir -p "$plans_dir"

    local dest=$(get_mirror_dest "$file_path" "$plans_dir")

    if [[ -z "$dest" ]]; then
        local repo_name=$(basename "$(pwd)" | tr '[:upper:]' '[:lower:]' | tr ' -' '_' | tr -cd '[:alnum:]_')
        local timestamp=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
        local dest_filename="${repo_name}_${timestamp}.md"
...
main() {
    ...
    local file_path=$(read_plan)
    [[ -z "$file_path" ]] && return

    local dest=$(get_plan_path "$file_path")
